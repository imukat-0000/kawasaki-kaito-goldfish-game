...
- 公開ページは5秒ごとにカウンターAPIを取得し直す。自動更新は `src` を送らない取得のみなのでカウントを増やさず、別端末から増えた値を常時表示端末へ反映する。`counter-test.html`、`sample`、ローカルの `demo` 表示では自動更新しない。
...
- スプレッドシートで `nfc_count` を管理する。
- `?src=nfc` のアクセス時だけ値を1増やす。
- `src` がないアクセスでは加算せず、現在値だけを返す。
- JSON形式は `{ nfc: number, total: number }`。

## NFCカウント

- NFCタグには本体ホームページ `https://kaitokawasaki.com/?visit=nfc` を設定する。ホームページ側で `visit=nfc` を受け取り、カウンターAPIへ `src=nfc` を送る。
- 同じ端末でもNFCをタッチしてページを開くたびに加算する。日次・端末別の重複防止は行わない。
- Apps Scriptのロックで同時アクセスの加算競合を防ぐ。
- APIは10秒でタイムアウトし、通信失敗時は端末内の最終正常値を明示付きで表示する。

## 成長ゲーム仕様

- 1アクセスにつき現在育成中の生物が1段階成長する。表示数を直接1匹増やす仕様ではない。
- NFC経由のAPI加算が成功した直後に「Thank you !」を約2秒表示する。取得のみ、サンプル表示、通信失敗時には表示しない。
- カエル以外の成長サイクルは5カウントで完了する。通常金魚と出目金の出現は、各10体内で通常金魚5体・出目金5体となるよう固定的にシャッフルされるため、交互にはならず、再表示しても変わらない。通常の金魚は5回目に鯉へ進化することが基本だが、約22%で鯉になれず大きな金魚として桶に残る。結果はサイクル番号から固定的に決まり、再表示しても変わらない。
- 成長途中の出目金は赤・黒のみ。出目金は鯉へ進化せず、5回目は通常なら成長途中と同じ赤または黒のまま大きな出目金になる。20%でレア進化となり、金色 `assets/demekin_gold.png`、宝石のようにきらめく青紫 `assets/demekin_jewel.png`、緑青色 `assets/demekin_verdigris.png` のいずれかへ変化する。鯉・大きな金魚・大きな出目金はいずれも代表表示を直近24体までに抑える。
...
- 次の進化までの進捗バーを表示する。金魚・出目金は5段階、カエルは8段階。
- 画面内にNFCタッチ数を表示する。
...
- `script.js`: API通信、NFCタッチの加算、成長、鯉・カエル、レア来訪者、背景花火、ステージ処理
...
本番カウントを変更せずに操作確認する場合は、次の専用ページを開く。NFCタッチの加算、任意値入力、次の節目、全ステージへの移動、人物の登場・退場、リセットができ、テスト値はブラウザ内だけに保存される。
