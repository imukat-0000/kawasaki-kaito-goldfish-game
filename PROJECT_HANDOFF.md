...
- 子ども二人は左右それぞれの専用4フレームで歩いて現れ、桶の後方、水面中央より上の位置で4フレームの後ろ向きしゃがみ姿へ切り替わる。成人女性は立ったまま桶を見る。
- 約8.5〜12.5秒見たあと、子どもたちは左右どちらかへ歩いて画面外へ退場する。再登場は約9〜18秒後で、常時表示にはしない。
- 透過シートは `assets/human-sprites/sheets/`、左歩行・後ろ姿の追加シートは `assets/human-sprites/sheets/directions/`、実装用フレームは `assets/human-sprites/frames/` に保存する。再切り出しは `tools/slice_human_sprites.py` を使う。
...
- ズームアウト時は通常背景を等倍から80%へ縮小しながら、先読み済みのステージ別広場を外側から見せる。ズームイン時は通常背景を80%から等倍へ戻して最後に完全表示する逆再生にし、異なる楼閣や提灯が長時間半透明で重ならないようにする。
- 初期HTMLの背景は通常の `assets/tub_base.png` に揃える。ズームアウト画像を初期表示すると、API取得後の最初の描画で画角が跳ねるため使用しない。
- 人物は背景より手前、HUDより奥に置く。ステージ2〜6の手前灯籠は `.people-foreground` でズームアウト背景の同じ画素を楕円状に再表示し、男女の足や裾より手前に見せる。別素材を重ねず、元背景と色・光・ピクセル位置を完全一致させる。
- 女の子は桶を見る `watch-back` の時だけ画像を左右反転して左向きにする。歩行中は左右別フレームの方向を維持する。
