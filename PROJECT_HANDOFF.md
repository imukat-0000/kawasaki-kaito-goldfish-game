...
- 画面負荷を抑えるため、金魚の代表表示は最大72体。鯉は1,000匹以降に別枠で追加し、最終段階で最大10体。
- 合計値と進捗表示は実数を維持する。
- 魚の種類・向き・位置は魚番号だけを種にした決定的な乱数を使い、カウント増加時も既存魚を変更しない。
- 魚の位置は番号ごとの決定的な乱数で桶内の楕円範囲へ配置する。中央から外側へ順番に広がる配置にはしない。
...
- `assets/animal-sprites/`: ゲームボーイ風の猫、鳥、おたまじゃくし、カエル、成長変化の透過スプライトシートと個別フレーム。
