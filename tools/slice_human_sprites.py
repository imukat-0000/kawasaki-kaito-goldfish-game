...
STAND_REACH_SHEETS = tuple(
    (character, f"{character}-stand-reach.png") for character in DIRECTION_CHARACTERS
)
...
def main() -> None:
    ...
    for character, filename in STAND_REACH_SHEETS:
        sheet = Image.open(SHEET_ROOT / filename).convert("RGBA")
        cell_width = sheet.width // 2
        cell_height = sheet.height // 2
        output = FRAME_ROOT / character
        ...
        for index in range(4):
            ...
            frame.save(output / f"stand-reach-{index + 1:02d}.png", optimize=True)
