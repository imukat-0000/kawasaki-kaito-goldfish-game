...
公開ページを開いたままにした端末は、15秒ごとに共有カウントを再取得して自動更新します。この再取得には `src` を含めないため、カウントは増えません。テストページ、`sample`、ローカルの `demo` 表示では自動更新しません。
...
- おたまじゃくしは7段階でカエルに近づき、8回目で成体と同じ大きさの泳ぐカエルへ切り替わる。カエル系の成長ゲージは8段階で満タンになり、内部周期は10回目で次の育成へ移る
